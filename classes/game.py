...
class Person:
    ...
    def choose_enemy_spell(self):
        magic_choice = random.randrange(0, len(self.magic))  # ???? i czemu nie len(enemy.magic) - 1 ????
        spell = self.magic[magic_choice]
        magic_dmg = spell.generate_damage()

        pct = self.hp / self.maxhp * 100  # percent of their hp

        if self.mp < spell.cost or spell.type == "white" and pct > 50:
            self.choose_enemy_spell()
        else:
            return spell, magic_dmg  # sprawdzić co to jest return lol
